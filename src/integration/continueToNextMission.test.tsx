...
describe('The Continue to Next Mission CTA', () => {
  it('is absent before a mission completes, then loads the next mission when clicked', async () => {
    ...
    const continueButton = await screen.findByRole('button', { name: 'Continue to District Ties' })
    ...
    expect(screen.getByRole('heading', { name: 'District Ties' })).toBeInTheDocument()
  })
})
