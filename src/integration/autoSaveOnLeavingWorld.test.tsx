...
// specifically about the classic dashboard's question console — pre-seed
// the flag (as a returning player would have) and switch views via the
 ...
 * Vitest's Canvas mock; the question mission flow below exercises the
 * exact same auto-save mechanism (the same unmount effect, calling the
 * same saveCurrentGame) without needing that. The lesson-specific version
 * of this exact scenario is covered in Playwright (e2e/world-scene-3d.spec.ts).
 */
describe('Meridian 1.0 closeout — auto-save on leaving /world (unmount)', () => {
  it('persists a completed mission even when the player never clicks Save, just navigates away', async () => {
    ...
    // First Contact is now "הקיסר הראשון" (The First Emperor) — a History
    // multiple-choice question; option 0 (אוגוסטוס) is the correct answer
    // (see missions/firstContact.ts).
